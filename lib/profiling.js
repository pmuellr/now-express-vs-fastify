...
// Alterative (and replacement?) for @no-pro/express-middleware .
// https://www.npmjs.com/package/@no-pro/express-middleware
...
function getProfilingMiddleware () {
  ...
  return async (req, res, next) => {
    const reqProfilingKey = req.headers['x-no-pro-profiling-key']
    ...
    const putURL = req.headers['x-no-pro-http-put']
    if (putURL == null) {
    ...
    res.end = async (...args) => {
      ...
      await writeProfile(req, res, profile, putURL)
...
async function writeProfile (req, res, profile, putURL) {
